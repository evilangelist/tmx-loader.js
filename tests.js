...
function runTests(){
	...
	try{
		...
		checkEquals("Map Height", tmxloader.map.height,"100");
	...
	try{
		...
		checkEquals("Tileset Src",tmxloader.map.tilesets[0].src,"../Prion/assets/spritesheet.png");	
	...
	checkEquals("Layer 1 Name", tmxloader.map.layers[0].name, "Tile Layer 1");
	checkEquals("Layer1 properties['display']", tmxloader.map.layers[0].properties['display'], "true");
	
	checkEquals("Layer1 data [4,25]", tmxloader.map.layers[0].data[25][4], 1);
	...
	checkEquals("Layer2 data [4,25]", tmxloader.map.layers[1].data[25][4], 65);
	...
	try{	
		...
		checkEquals("Check Number of Enemy Objects", tmxloader.map.objectgroup['Enemies'].objects.length,"2");
		...
		checkEquals("Check Enemy Object #1 - x", tmxloader.map.objectgroup['Enemies'].objects[0].x,670);
		checkEquals("Check Enemy Object #1 - y", tmxloader.map.objectgroup['Enemies'].objects[0].y,740);
		...
		checkEquals("Check Enemy Object #2 - x", tmxloader.map.objectgroup['Enemies'].objects[1].x,782);
		checkEquals("Check Enemy Object #2 - y", tmxloader.map.objectgroup['Enemies'].objects[1].y,427);
